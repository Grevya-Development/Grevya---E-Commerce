  ...
  useEffect(() => {
    ...
    const fetchOrders = async () => {
      const { data, error } = await supabase
        .from("orders")
        .select(
          "id, created_at, total_amount, status, order_status, payment_status, order_items(product_name,quantity)",
        )
        .eq("user_id", user.id)
        ...
        <div className="container mx-auto px-4">
          ...
          {loading ? (
            ...
            <div className="rounded-[2.5rem] bg-white/70 backdrop-blur-md p-10 md:p-14 text-center border border-[#A68D65]/20 shadow-sm max-w-xl mx-auto relative overflow-hidden group">
              ...
              <h2 className="font-serif text-3xl font-bold text-[#33381C] mb-2">
                No orders recorded
              </h2>
              <p className="mx-auto mt-2 max-w-sm text-neutral-500 text-sm leading-relaxed font-medium mb-8">
                Once you complete checkout, your order updates and delivery
                tracker will appear here.
              </p>
              ...
              <div className="bg-[#F7EEE4]/40 border border-[#A68D65]/15 rounded-2xl p-5 mb-8 text-left">
                ...
                <div className="flex gap-2">
                  ...
                  <Button
                    onClick={() => {
                      alert(
                        "This order tracking query is processing. Please check your email for the verified live tracking link.",
                      );
                    }}
            ...
            <div className="space-y-4">
              {orders.map((order) => (
                <Link
                  key={order.id}
                  to={`/orders/${order.id}`}
                  className="flex flex-col justify-between gap-4 rounded-2xl bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md sm:flex-row sm:items-center"
                >
                  <div>
                    ...
                    <p className="text-sm text-neutral-700 mt-1">
                      {(() => {
                        ...
                        if (items.length === 0) return "Items unavailable";
                        if (items.length === 1)
                          return items[0].product_name || "Items unavailable";
                        const first = items[0].product_name || "Item";
                        return `${first} + ${items.length - 1} more items`;
                      })()}
                    </p>
                  ...
                  <div className="flex flex-wrap items-center gap-3">
                    <span className="rounded-full bg-green-50 px-3 py-1 text-xs font-bold capitalize text-green-800">
                      {order.status || order.order_status || "pending"}
                    </span>
                    <span className="rounded-full bg-cream-light px-3 py-1 text-xs font-bold capitalize text-green-800">
