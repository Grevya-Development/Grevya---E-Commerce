...
// Unambiguous date formatter — replaces "2/7/2026" style output
...
export default function PendingProducts() {
  ...
  // Withdraw confirmation dialog state (replaces window.confirm)
  const [confirmWithdrawOpen, setConfirmWithdrawOpen] = useState(false);
  const [productToWithdraw, setProductToWithdraw] =
  ...
  // Opens the in-app confirmation dialog instead of window.confirm
  const requestWithdraw = (product: PendingProduct) => {
    setProductToWithdraw(product);
    setConfirmWithdrawOpen(true);
  };

  // Runs the actual delete once the user confirms in the dialog
  const confirmWithdraw = async () => {
    if (!productToWithdraw) return;

    const productId = productToWithdraw.id;
      ...
      <div className="p-8">
        ...
        <div className="grid gap-4 md:grid-cols-3 mb-6">
          ...
          <div className="rounded-3xl border border-green-100 bg-white p-6 shadow-sm">
            ...
            <p className="mt-3 text-4xl font-semibold text-green-900">
              {isSearching ? filteredProducts.length : products.length}
            </p>
            <p className="mt-2 text-sm text-gray-500">
              {isSearching
                ? "Matching pending products."
                : "Showing all pending products."}
            </p>
          </div>

          {/* Latest request card — was oversized purple heading, now matches theme */}
          <div className="rounded-3xl border border-green-100 bg-white p-6 shadow-sm">
            <p className="text-sm font-medium text-gray-500">Latest request</p>
            <p
              className="mt-3 text-lg font-semibold text-green-900 truncate"
              title={products[0]?.name ?? undefined}
            >
              {products[0]?.name ?? "No pending requests"}
            </p>
            <p className="mt-2 text-sm text-gray-500">
              {products[0]?.created_at
                ? `Requested ${formatDate(products[0].created_at)}`
                  ...
                  >
                    ...
                    <td className="px-6 py-4 text-right text-sm">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        <button
                          onClick={() => requestWithdraw(product)}
        ...
        {/* Rejection reason dialog (existing) */}
        <Dialog
        ...
        {/* Withdraw confirmation dialog (replaces window.confirm) */}
        <Dialog
          open={confirmWithdrawOpen}
          onOpenChange={(open) => {
            setConfirmWithdrawOpen(open);
            if (!open) setProductToWithdraw(null);
          }}
        >
          <DialogContent className="max-w-md">
            <DialogHeader>
              <DialogTitle>Withdraw product request?</DialogTitle>
              <DialogDescription>
                {productToWithdraw
                  ? `"${productToWithdraw.name}" will be permanently removed from your pending requests. This cannot be undone.`
