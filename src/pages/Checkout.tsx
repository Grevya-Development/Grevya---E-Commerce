...
const normalizeAddressForDB = (info: any, userId: string) => {
  ...
  return {
    ...
    address_line_2:
      (
        info.addressLine2 +
        (info.landmark ? `, Landmark: ${info.landmark}` : "")
      ).trim() || null,
    address_line2:
      (
        info.addressLine2 +
...
const safeInsertOrder = async (orderPayload: any) => {
  ...
  while (true) {
    ...
    if (missingColumn && missingColumn in attemptData) {
      console.warn(
        `[Grevya Dev Resilience] Column '${missingColumn}' not found in orders database. Retrying order save without it.`,
      );
...
const safeInsertOrderItems = async (itemsPayload: any[]) => {
  ...
  while (true) {
    ...
    if (missingColumn) {
      ...
      if (keyExisted) {
        console.warn(
          `[Grevya Dev Resilience] Column '${missingColumn}' not found in order_items database. Retrying insert.`,
        );
        if (
          attemptData.length === 0 ||
          Object.keys(attemptData[0]).length === 0
        ) {
          return { error };
        }
        continue;
...
const safeInsertNotification = async (notifPayload: any) => {
  ...
  while (true) {
    ...
    if (missingColumn && missingColumn in attemptData) {
      console.warn(
        `[Grevya Dev Resilience] Column '${missingColumn}' not found in notifications database. Retrying notification insert without it.`,
      );
      delete attemptData[missingColumn];
      if (Object.keys(attemptData).length === 0) {
        return { error };
...
const safeInsertAddress = async (addressPayload: any) => {
  ...
  while (true) {
    ...
    if (missingColumn && missingColumn in attemptData) {
      console.warn(
        `[Grevya Dev Resilience] Column '${missingColumn}' not found in addresses database. Retrying address save without it.`,
      );
      delete attemptData[missingColumn];
      if (Object.keys(attemptData).length === 0) {
        return { data: null, error };
  ...
  useEffect(() => {
    const fetchSavedAddresses = async () => {
      ...
      try {
        ...
        const parsed = (data || []).map((addr: any) =>
          normalizeAddressForUI(addr),
        );
  ...
  const validateStep1 = () => {
    ...
    if (
      ...
      toast({
        ...
        description:
          "Please complete all required delivery information fields.",
        variant: "destructive",
      });
      return false;
    }
        ...
        <main className="flex-grow bg-cream/30 py-16 flex items-center justify-center">
          <div className="text-center bg-white p-8 rounded-3xl shadow-sm border border-neutral-100 max-w-sm w-full mx-4">
            ...
            <p className="mt-2 text-sm text-neutral-600">
              Please sign in to use secure checkout.
            </p>
          ...
          <div className="flex items-center justify-between max-w-lg mx-auto mb-12 w-full bg-white p-4 rounded-2xl shadow-sm border border-neutral-100/50">
            {[
              ...
              <React.Fragment key={s.num}>
                {idx > 0 && (
                  <div
                    className={`flex-1 h-0.5 mx-2 ${step > idx ? "bg-[#33381C]" : "bg-neutral-100"}`}
                  />
                )}
                <button
                  type="button"
                  onClick={() => {
                    ...
                    else if (s.num === 2 && step === 1) {
                      if (validateStep1()) paginate(2);
                ...
                >
                  <div
                    className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-xs transition-colors ${
                      step >= s.num
                        ? "bg-[#33381C] text-white shadow-md shadow-[#33381C]/10"
          ...
          <div className="mx-auto grid max-w-6xl gap-8 lg:grid-cols-[1.35fr_0.85fr]">
            <div>
              ...
              <p className="mb-8 text-neutral-500">
                Secure delivery details, payment, and order confirmation.
              </p>
                      ...
                      {user && savedAddresses.length > 0 && (
                        <div className="space-y-3">
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {savedAddresses.map((addr) => (
                              ...
                              >
                                <div className="flex items-center justify-between mb-2">
                                  <span className="font-bold text-neutral-800 text-xs capitalize">
                                    [{addr.label || "Address"}]
                                  </span>
                                  {addr.is_default && (
                            ...
                            >
                              <span className="text-sm font-bold text-[#33381C]">
                                + Use a new address
                              </span>
                              <span className="text-xs text-neutral-400 mt-1 text-center font-medium">
                                Enter a custom shipping destination
                              </span>
                            </motion.div>
                          </div>
                        </div>
                        ...
                        {paymentMethods.map((method) => (
                          ...
                          >
                            <input
                              ...
                              className="sr-only"
                            />
                            <div className="flex-shrink-0 mr-4 text-[#33381C]">
                              {method.icon}
                            </div>
                    ...
                    >
                      <div>
                        ...
                        <p className="text-sm text-neutral-400">
                          Please confirm that all shipping and payment
