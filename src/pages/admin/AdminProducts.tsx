...
export default function AdminProducts() {
  ...
  const toggleFeatured = async (id: string, current: boolean) => {
    ...
    } else if (!data || data.is_featured !== !current) {
      toast({
        title: "Update failed",
        description: "Supabase did not return the updated featured status.",
        variant: "destructive",
      });
    } else {
      toast({
  ...
  const toggleHidden = async (id: string, current: boolean) => {
    ...
    } else if (!data || data.is_hidden !== !current) {
      toast({
        title: "Update failed",
        description: "Supabase did not return the updated visibility status.",
        variant: "destructive",
      });
    } else {
      toast({
          ...
          {loading ? (
            ...
            <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
              {filteredProducts.map((product) => (
                <Card
                  key={product.id}
                  className="border-slate-200 bg-white transition-transform duration-300 ease-out hover:-translate-y-1 hover:shadow-xl"
                >
                  ...
                  <CardFooter className="flex flex-wrap gap-2">
                    ...
                    <Button
                      ...
                      disabled={actionLoading === product.id}
                      className="min-w-[105px] border-red-200 text-red-600 hover:bg-red-50 hover:text-red-700"
                    >
        ...
        <AlertDialogContent className="max-w-md">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              from the approved product catalog. This action cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
