...
export default function AdminNotifications() {
  ...
  const fetchRecipientCounts = async () => {
    const [{ count: sellers }, { count: users }] = await Promise.all([
      supabase
        .from("profiles")
  ...
  const hasActiveFilters =
    search.trim() || filterType !== "all" || filterRead !== "all" || filterDate !== "all";
  ...
  const sendNotification = async () => {
    ...
    try {
      ...
      if (sendTo === "individual") {
        userIds = [recipientId.trim()];
      } else {
        const role = sendTo === "sellers" ? "seller" : "user";
      ...
      if (!userIds.length) {
        toast({
          ...
          description: "Try a different audience or refresh recipient counts.",
        });
        return;
      }
      ...
      toast({
        ...
        description: `${userIds.length} recipient(s) will receive it.`,
      });
      closeSendModal();
