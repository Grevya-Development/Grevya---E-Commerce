  ...
  useEffect(() => {
    const fetchFeatured = async () => {
      try {
        ...
        // Fetch review rows for these featured products so ratings can be shown
        ...
        // Normalize data and attach review stats
        const formatted = (data || []).map((item) => ({
          ...item,
          image: item.image_url,
          rating:
            reviewStatsByProductId.get(item.id)?.averageRating ??
            item.rating ??
            0,
        ...
        {isLoading ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {[1, 2, 3, 4].map((n) => (
              <div
                key={n}
                className="bg-white rounded-2xl shadow-sm border border-neutral-100 flex flex-col h-[480px] overflow-hidden animate-pulse"
              >
