...
const NotificationBell = () => {
  ...
  const fetchNotifications = async () => {
    ...
    try {
      ...
      const notifs = data
        ? (data as Array<Notification & { is_read?: boolean }>).map(
            (notification) => ({
              ...notification,
      ...
      const orderReferences = Array.from(
        new Set(
          notifs
            ...
            .map((notification) =>
              orderReferenceFromGenericMessage(notification.message),
            )
      ...
      if (orderReferences.length > 0) {
        ...
        const ordersByReference = new Map(
          ((orderRows || []) as OrderWithItems[]).map((order) => [
            order.order_number,
            order,
          ]),
        );
        enrichedNotifications = notifs.map((notification) => {
          const reference = orderReferenceFromGenericMessage(
          ...
          return order
            ? {
                ...notification,
                message: formatOrderPlacedMessage(
                  reference!,
                  order.order_items,
                ),
              }
  ...
  useEffect(() => {
    ...
    // Fallback for projects where Supabase Postgres Changes has not yet been
    // enabled for the notifications table. This keeps the bell badge current
    // everywhere in the app without a browser refresh.
    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button
          variant="ghost"
          size="icon"
          className="relative hover:bg-[#A68D65]/10 text-gray-750"
        >
      ...
      <DropdownMenuContent
        align="end"
        className="w-80 p-0 border border-[#A68D65]/25 bg-white rounded-2xl overflow-hidden shadow-xl"
      >
        <motion.div
        ...
        >
          <div className="flex items-center justify-between px-4 py-3 border-b border-[#A68D65]/10 bg-[#F7EEE4]/30 select-none">
            <h3 className="font-serif text-sm font-bold text-[#33381C]">
              Notifications
            </h3>
            {unreadCount > 0 && (
              <button
          ...
          <div className="max-h-80 overflow-y-auto no-scrollbar">
            {notifications.length === 0 ? (
              <div className="py-10 px-4 text-center space-y-3">
                ...
                <div>
                  <p className="text-xs font-bold text-[#33381C] uppercase tracking-wider">
                    All caught up
                  </p>
                  <p className="text-[10px] text-neutral-400 font-semibold mt-0.5">
                    We'll alert you when orders ship or stock arrives.
                  </p>
                </div>
              </div>
            ) : (
              ...
              >
                ...
                {today.length > 0 && (
                  <div>
                    <div className="bg-[#FBF9F6] px-4 py-1 text-[9px] font-bold text-neutral-400 uppercase tracking-widest border-y border-gray-100/50">
                      Today
                    </div>
...
const NotificationItem = ({
  ...
  return (
    ...
    >
      <div
        className={`mt-0.5 p-1.5 rounded-xl ${notification.type === "order" ? "bg-emerald-100 text-[#33381C]" : "bg-[#F7EEE4] text-[#A68D65]"}`}
      >
